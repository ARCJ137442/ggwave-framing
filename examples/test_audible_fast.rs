...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== 完整报告 AUDIBLE_FAST 端到端测试 ===\n");
    ...
    let codec = GGWaveCodec::with_protocol(protocols::AUDIBLE_FAST)?;
    ...
    let wav_path = output_dir.join("AUDIBLE_FAST_full.wav");
    {
    ...
    drop(samples_f32);
    drop(wav_data);
    println!("  音频缓冲: {} bytes", audio_bytes.len());
